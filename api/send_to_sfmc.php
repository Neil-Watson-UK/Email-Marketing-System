...
// Salesforce Marketing Cloud credentials — set via environment or config.local.php
define('SFMC_CLIENT_ID', getenv('SFMC_CLIENT_ID') ?: '');
define('SFMC_CLIENT_SECRET', getenv('SFMC_CLIENT_SECRET') ?: '');
define('SFMC_AUTH_BASE_URI', rtrim(getenv('SFMC_AUTH_BASE_URI') ?: '', '/'));
define('SFMC_REST_BASE_URI', rtrim(getenv('SFMC_REST_BASE_URI') ?: '', '/'));

if (SFMC_CLIENT_ID === '' || SFMC_CLIENT_SECRET === '' || SFMC_AUTH_BASE_URI === '' || SFMC_REST_BASE_URI === '') {
    header('Content-Type: application/json');
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'SFMC credentials are not configured.']);
    exit;
}
